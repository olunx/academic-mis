...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td valign="top" background="<%=path %>/content/images/admin/content-bg.gif"><table width="100%" height="31" border="0" cellpadding="0" cellspacing="0" class="left_topbg" id="table2">
      <tr>
        <td height="31"><div class="titlebt">修改活动类型</div></td>
      </tr>
    </table></td>
    ...
    <td valign="top" bgcolor="#F7F8F9"><table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
      ...
      <tr>
        ...
        <td width="100%" valign="top">
        	<div class="context">
      		<form action="<%=path %>/activitytype/modifyActivityType" method="post">
      			<input type="hidden" name="atDto.id" value="${activityType.id }">
				学术活动名称：<input type="text" name="atDto.name" value="${activityType.name }"/><br/>
				学术活动级别：<select name="atDto.level" size="1">
								<option value="1" <c:if test="${activityType.level == 1 }">selected="selected"</c:if> >院级</option>
								<option value="2" <c:if test="${activityType.level == 2 }">selected="selected"</c:if> >校级</option>
								<option value="3" <c:if test="${activityType.level == 3 }">selected="selected"</c:if> >省级</option>
								<option value="4" <c:if test="${activityType.level == 4 }">selected="selected"</c:if> >国家级</option>
								<option value="5" <c:if test="${activityType.level == 5 }">selected="selected"</c:if> >国际级</option>
							</select><br />
				学术活动简介：<br/>
				<textarea id="comment" name="atDto.intro" cols="100%" rows="10" >${activityType.intro }</textarea><br/>
