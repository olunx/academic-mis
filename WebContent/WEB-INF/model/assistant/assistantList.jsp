        ...
        <td width="100%" valign="top">
        	<div class="context">
      			<c:choose>
					...
					<c:otherwise>
						<form method="post" onSubmit="post(this);return false;" action="<%=path%>/assistant/deleteManyAssistant">
						<table class="table">
							<tr>
							...
							<c:forEach items="${pageBean.list}" var="assistant">
								<tr>
									<td><input type="checkbox" name="id" value="${assistant.id}" /></td>
									<td>${assistant.username}</td>
									<td>${assistant.realName}</td>
									<td><a href="<%=path%>/assistant/goModifyAssistant?id=${assistant.id }&page=${page}" class="btn_edit">编辑</a></td>
									<td><a href="<%=path%>/assistant/deleteAssistant?id=${assistant.id }&page=${page}" class="btn_del">删除</a></td>
								</tr>
							</c:forEach>
						</table>
						...
						<c:choose>
							...
							<c:otherwise>
								<a target="content" href="<%=path%>/assistant/listAssistant?page=1"><span>首页</span></a>
								<a target="content" href="<%=path%>/assistant/listAssistant?page=${pageBean.currentPage-1}"><span>上一页</span></a>
							</c:otherwise>
						</c:choose> <c:choose>
							<c:when test="${pageBean.currentPage != pageBean.totalPage}">
								<a target="content" href="<%=path%>/assistant/listAssistant?page=${pageBean.currentPage+1}"><span>下一页</span></a>
								<a target="content" href="<%=path%>/assistant/listAssistant?page=${pageBean.totalPage}"><span>尾页</span></a>
							</c:when>
							<c:otherwise>
								<a><span>下一页</span></a>
								<a><span>尾页</span></a>
